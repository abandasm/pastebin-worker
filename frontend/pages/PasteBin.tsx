...
export function PasteBin({ config }: { config: Env }) {
  ...
  function onStartDelete() {
    startDelete(async () => {
      try {
        ...
        if (resp.ok) {
          showModal("删除成功", "删除结果可能需要最多 60 秒才会在全球节点同步。")
          setPasteResponse(undefined)
          setPasteSetting({ ...pasteSetting, uploadKind: "short", manageUrl: "" })
        } else {
          await handleFailedResp("删除失败", resp)
        }
      } catch (e) {
        handleError("删除失败", e as Error)
      }
    })
  }
  ...
  const info = (
    <div className="mx-4 lg:px-4 lg:mx-0">
      ...
      <p className="my-2">一个运行在 Cloudflare Workers 上的临时文本和文件分享工具。</p>
      <p className="my-2">
        <b>用法</b>：粘贴文本或拖入文件，上传后分享生成的链接。你也可以使用{" "}
      ...
      <p className="my-2">
        <b>提醒</b>：这里只适合临时分享<b>（最长 {getMaxExpirationReadable(config)}）</b>。文件可能会在到期后自动删除。
      </p>
    </div>
  )
